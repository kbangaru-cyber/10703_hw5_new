...
class PENN(nn.Module):
    ...
    def get_loss(self, targ, mean, logvar):
        ...
        # NLL = 0.5 * log(2π) + 0.5 * logvar + 0.5 * (targ - mean)^2 / exp(logvar)

        inv_var = torch.exp(-logvar)
        mse_loss = (targ - mean) ** 2
        loss = 0.5 * (np.log(2 * np.pi) + logvar + mse_loss * inv_var)

        # I need to do sum over the state dimensions and avaergae over the batch
        ...
        for itr in range(num_train_itrs):
            ...
            for net_idx in range(self.num_nets):
                batch_indices = np.random.choice(
                    num_samples, size=batch_size, replace=True
                )
                batch_inputs = inputs[batch_indices]
                batch_targets = targets[batch_indices]

                raw_output = self.networks[net_idx](batch_inputs)
