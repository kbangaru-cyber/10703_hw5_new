...
class MPC:
    def __init__(
        ...
        self.acs_buff = np.array([]).reshape(
            0, self.action_dim
        )  # planned action buffer
        self.prev_sol = np.tile(
            np.zeros(self.action_dim), self.plan_horizon
        )  # size: (action_dim * plan_horizon,)
        self.init_var = np.tile(
            np.ones(self.action_dim) * (0.5**2), self.plan_horizon  # std = 0.5
        )  # size: (action_dim * plan_horizon,)
    ...
    def obs_cost_fn(self, states):
        ...
        return (
            W_PUSHER * np.maximum(d_box - 0.4, 0)
            + W_GOAL * d_goal
            + W_DIFF * diff_coord
        )
    ...
    def get_action_cost(self, ac_seqs):
        ...
        for t in range(self.plan_horizon):
            ...
            delta_costs = self.obs_cost_fn(next_obs).reshape(
                [popsize, -1]
        ...
        return np.mean(
            np.where(
                np.isnan(total_costs), 1e6 * np.ones_like(total_costs), total_costs
            ),
            axis=1,
        )
    ...
    def predict_next_state_model(self, states, actions):
        ...
        # Repeat each action num_particles times to match states
        # actions: (popsize, action_dim) -> (popsize * num_particles, action_dim)
        actions_expanded = np.repeat(actions, self.num_particles, axis=0)

        # Concatenate states and actions as input to the model
        # Input shape: [popsize * num_particles, state_dim + action_dim]
        model_input = np.concatenate([states, actions_expanded], axis=-1)

        # Convert to tensor
        model_input_tensor = torch.tensor(
            model_input, dtype=torch.float, device=self.model.device
        )

        # TS1: For each particle, randomly select a network from ensemble
        # Vectorized version: randomly assign network indices and batch process
        num_samples = model_input_tensor.shape[0]
        network_assignments = np.random.randint(0, self.num_nets, size=num_samples)

        # Initialize array for next states
        next_states = np.zeros((num_samples, self.state_dim))

        # Process each network's assigned samples in batch
        for net_idx in range(self.num_nets):
            # Find all samples assigned to this network
            mask = network_assignments == net_idx
            if not np.any(mask):
                continue

            # Get predictions for all samples assigned to this network
            batch_input = model_input_tensor[mask]
            raw_output = self.model.networks[net_idx](batch_input)
            mean, logvar = self.model.get_output(raw_output)

            # Sample delta from Gaussian N(mean, exp(logvar))
            std = torch.sqrt(torch.exp(logvar))
            delta = torch.normal(mean, std)

            # Add delta to current states (model predicts change, not absolute state)
            next_states[mask] = states[mask] + delta.detach().cpu().numpy()
    ...
    def predict_next_state_gt(self, states, actions):
        ...
        # TODO: write your code here
        # states shape: [popsize * num_particles, state_dim]
        # actions shape: [popsize, action_dim]
        # Need to expand actions to match states (repeat for each particle)

        # Repeat each action num_particles times to match states
        # actions: (popsize, action_dim) -> (popsize * num_particles, action_dim)
        actions_expanded = np.repeat(actions, self.num_particles, axis=0)

        # Use environment's ground truth dynamics for each state-action pair
        next_states = []
        # Get unwrapped env to access get_nxt_state
        env_unwrapped = self.env.unwrapped if hasattr(self.env, 'unwrapped') else self.env
        for state, action in zip(states, actions_expanded):
            # Note: function name is get_nxt_state (missing 'e')
            # Convert to numpy arrays to ensure proper formatting
            next_state = env_unwrapped.get_nxt_state(
    ...
    def act(self, state, t):
        ...
        if self.use_mpc:
            ...
            self.acs_buff = self.optimizer.solve(self.prev_sol, self.init_var).reshape(
                -1, self.action_dim
            )
