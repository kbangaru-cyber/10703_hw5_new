...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, max_size=10000, save_timestep=False):
        ...
        if self.save_timestep:
            self.timestep = np.zeros((max_size, 1))

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def add(self, state, action, next_state, reward, done, timestep=None):
    ...
    def sample(self, batch_size):
        ...
        if self.save_timestep:
            # return the sampled timesteps, not the write pointer
            out.append(self.timestep[ind].copy())
        return out
